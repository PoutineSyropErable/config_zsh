...
def open_nvim(remote_session_name: str, files: List[str]):
    ...
    command = [
        ...
        "--cmd",
        f"NvimPossessionLoadOrCreate '{remote_session_name}'",
    ]
...
if __name__ == "__main__":
    # Create the parser
    parser = argparse.ArgumentParser(description="Open Neovim with session management.")
    ...
    ret = open_nvim(args.name, args.files)
    exit(ret)
...
def get_session_file(remote_session_name: str):
    ...
    session_file = os.path.join(session_dir, remote_session_name + ".vim")  # Ensure it has the .vim extension
